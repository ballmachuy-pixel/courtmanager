    ...
    <div className="space-y-6 animate-in pt-4">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
         ...
         <div className="flex gap-2 w-full md:w-auto">
            <Skeleton className="h-10 flex-1 md:w-48 rounded-xl bg-white/5" />
            <Skeleton className="h-10 w-12 rounded-xl bg-white/5" />
         </div>
      </div>

      <div className="glass-card p-2 md:p-6 space-y-4 bg-slate-900/20 border-white/5">
         <div className="hidden md:flex justify-between mb-6 pb-4 border-b border-white/5">
           <Skeleton className="h-6 w-32 bg-white/5" />
           <Skeleton className="h-6 w-16 bg-white/5" />
         </div>
         
         {/* Mobile Friendly Row Skeletons */}
         {[1, 2, 3, 4, 5].map((i) => (
           <div key={i} className="flex items-center gap-4 p-4 rounded-2xl bg-white/[0.02] border border-white/5">
             ...
             <Skeleton className="w-16 h-8 rounded-full bg-white/5 hidden md:block" />
