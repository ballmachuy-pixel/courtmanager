...
export async function createClass(formData: FormData) {
  ...
  const maxStudentsStr = formData.get('max_students') as string;
  const maxStudents = maxStudentsStr ? parseInt(maxStudentsStr, 10) : 20;
  const coachIds = formData.getAll('coach_ids').map(id => id as string);

  if (!name) throw new Error('Vui lòng điền tên lớp');

  const { error } = await classService.createClass({
    name,
    ageGroup,
    skillLevel,
    maxStudents: isNaN(maxStudents) ? 20 : maxStudents,
    coachIds
  });
...
export async function updateClass(classId: string, formData: FormData) {
  ...
  if (!name) return { error: 'Vui lòng điền tên lớp' };

  const { error } = await classService.updateClass(classId, {
    name,
    ageGroup,
    skillLevel,
    maxStudents: isNaN(maxStudents) ? 20 : maxStudents,
...
export async function enrollStudent(studentId: string, classId: string) {
  return enrollStudents([studentId], classId);
}
...
export async function addSchedule(formData: FormData) {
  ...
  const { error } = await classService.addSchedules({
    ...
    endTime,
    location,
    locationId: formData.get('location_id') as string,
    coachIds
  });
