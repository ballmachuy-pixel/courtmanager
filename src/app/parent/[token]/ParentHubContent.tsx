...
export default function ParentHubContent({ parent, students, attendances, academy }: ParentHubContentProps) {
  const [activeStudentId, setActiveStudentId] = useState(students[0]?.id);
  
  const activeStudent = students.find(s => s.id === activeStudentId) || students[0];
  const activeAttendances = attendances.filter(a => a.student_id === activeStudentId);
    ...
    <div className="min-h-screen bg-slate-950 pb-20 animate-in">
      ...
      <div className="relative pt-16 pb-24 px-6 overflow-hidden">
        ...
        <div className="max-w-md mx-auto relative z-10 text-center">
          ...
          {students.length > 1 && (
            <div className="flex items-center justify-center gap-2 mb-8 p-1.5 bg-black/20 backdrop-blur-md rounded-2xl border border-white/10 max-w-xs mx-auto overflow-x-auto no-scrollbar">
              {students.map((s) => (
                <button
                  key={s.id}
          ...
          <div className="flex flex-col items-center">
            ...
            <h1 className="text-3xl font-black text-white mb-2 tracking-tight">{activeStudent.full_name}</h1>
            <div className="flex items-center gap-3">
              ...
              <span className="text-xs font-bold text-amber-300">Level: {activeStudent.skill_level ? (SKILL_LABELS[activeStudent.skill_level as keyof typeof SKILL_LABELS] || activeStudent.skill_level) : 'Tân binh'}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-md mx-auto px-6 -mt-12 space-y-6">
        ...
        <div className="grid grid-cols-2 gap-4">
           <div className="glass-card p-5 text-center flex flex-col items-center">
              <span className="text-3xl font-black text-emerald-400 mb-1">{presentCount}</span>
              <span className="text-[10px] font-black text-slate-500 uppercase tracking-tighter">Buổi tham gia</span>
           </div>
           <div className="glass-card p-5 text-center flex flex-col items-center">
              <span className="text-3xl font-black text-red-500 mb-1">{absentCount}</span>
                   ...
                   <div key={idx} className="flex items-center gap-4 p-4 hover:bg-white/5 rounded-2xl transition-colors">
                      ...
                      <div className="flex-1 min-w-0">
                         ...
                         <p className="text-[10px] text-slate-500 uppercase font-black truncate">
                            {(a as any).classes?.name || 'Buổi tập Basketball'}
                         </p>
                      </div>
